open Types_internal
open Aws
type input = RestoreObjectRequest.t
type output = RestoreObjectOutput.t
      ...
      Util.or_error (Util.option_bind resp RestoreObjectOutput.parse)
        (let open Error in
           BadResponse
             {
               body;
               message = "Could not find well formed RestoreObjectOutput."
             })
    with
            ...
            (BadResponse
               {
                 body;
                 message =
                   ("Error parsing RestoreObjectOutput - missing field in body or children: "
                      ^ msg)
               })
  with
  | Failure msg ->
