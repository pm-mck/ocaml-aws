...
let to_http service region req =
  let uri =
    Uri.add_query_params
      ...
      (List.append
         [("Version", ["2010-05-15"]); ("Action", ["DescribeStackEvents"])]
         (Util.drop_empty
           ...
           BadResponse
             {
               body;
               message =
                 "Could not find well formed DescribeStackEventsOutput."
             })
    with
            ...
            (BadResponse
               {
                 body;
                 message =
                   ("Error parsing DescribeStackEventsOutput - missing field in body or children: "
                      ^ msg)
               })
  with
  | Failure msg ->
