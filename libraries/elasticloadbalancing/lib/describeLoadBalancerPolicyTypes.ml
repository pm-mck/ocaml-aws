      ...
      (List.append
         ...
         (Util.drop_empty
            (Uri.query_of_encoded
               (Query.render
                  (DescribeLoadBalancerPolicyTypesInput.to_query req))))) in
  (`POST, uri, [])
let of_http body =
  try
    ...
    let resp =
      Util.option_bind
        ...
        (Xml.member "DescribeLoadBalancerPolicyTypesResult") in
    try
      Util.or_error
           ...
           BadResponse
             {
               body;
               message =
                 "Could not find well formed DescribeLoadBalancerPolicyTypesOutput."
             })
    with
            ...
            (BadResponse
               {
                 body;
                 message =
                   ("Error parsing DescribeLoadBalancerPolicyTypesOutput - missing field in body or children: "
                      ^ msg)
               })
  with
  | Failure msg ->
