  ...
  try
    ...
    let resp =
      Util.option_bind
        ...
        (Xml.member "DescribeMetricCollectionTypesResult") in
    try
      Util.or_error
           ...
           BadResponse
             {
               body;
               message =
                 "Could not find well formed DescribeMetricCollectionTypesAnswer."
             })
    with
            ...
            (BadResponse
               {
                 body;
                 message =
                   ("Error parsing DescribeMetricCollectionTypesAnswer - missing field in body or children: "
                      ^ msg)
               })
  with
  | Failure msg ->
